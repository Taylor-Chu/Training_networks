...
class UNet(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=2, act_mode='l', downsample_mode='strideconv', upsample_mode='convtranspose'):
        # [64, 128, 256, 512]
        ...
        self.m_downconv1 = B.sequential(*[B.conv(nc[0], nc[0], mode='C'+act_mode) for _ in range(nb)])
        self.m_downmax1 = B.sequential(B.conv(nc[0], nc[0], mode='M', kernel_size=2, stride=2), B.conv(nc[0], nc[1], mode='C'))
        self.m_downconv2 = B.sequential(*[B.conv(nc[1], nc[1], mode='C'+act_mode) for _ in range(nb)])
        self.m_downmax2 = B.sequential(B.conv(nc[1], nc[1], mode='M', kernel_size=2, stride=2), B.conv(nc[1], nc[2], mode='C'))
        self.m_downconv3 = B.sequential(*[B.conv(nc[2], nc[2], mode='C'+act_mode) for _ in range(nb)])
        self.m_downmax3 = B.sequential(B.conv(nc[2], nc[2], mode='M', kernel_size=2, stride=2), B.conv(nc[2], nc[3], mode='C'))
        ...
        self.m_uptran3 = B.sequential(B.conv(nc[3], nc[3], mode='T', kernel_size=2, stride=2, padding=0), B.conv(nc[3], nc[2], mode='C'))
        self.m_upconv3 = B.sequential(*[B.conv(nc[2], nc[2], mode='C'+act_mode) for _ in range(nb)])
        self.m_uptran2 = B.sequential(B.conv(nc[2], nc[2], mode='T', kernel_size=2, stride=2, padding=0), B.conv(nc[2], nc[1], mode='C'))
        self.m_upconv2 = B.sequential(*[B.conv(nc[1], nc[1], mode='C'+act_mode) for _ in range(nb)])
        self.m_uptran1 = B.sequential(B.conv(nc[1], nc[1], mode='T', kernel_size=2, stride=2, padding=0), B.conv(nc[1], nc[0], mode='C'))
        self.m_upconv1 = B.sequential(*[B.conv(nc[0], nc[0], mode='C'+act_mode) for _ in range(nb)])

        self.m_tail = B.conv(nc[0], out_nc, bias=True, mode='Cr')

    def forward(self, x0):
        x1 = self.m_head(x0)
        x2_conv = self.m_downconv1(x1)
        x2_max = self.m_downmax1(x2_conv)
        x3_conv = self.m_downconv2(x2_max)
        x3_max = self.m_downmax2(x3_conv)
        x4_conv = self.m_downconv3(x3_max)
        x4_max = self.m_downmax3(x4_conv)

        x_body = self.m_body(x4_max)

        x_up3_tran = self.m_uptran3(x_body)
        x_up3_conv = self.m_upconv3(x_up3_tran+x4_conv)
        x_up2_tran = self.m_uptran2(x_up3_conv)
        x_up2_conv = self.m_upconv2(x_up2_tran+x3_conv)
        x_up1_tran = self.m_uptran1(x_up2_conv)
        x_up1_conv = self.m_upconv1(x_up1_tran+x2_conv) + x1 # maybe x1, check again

        x_out = self.m_tail(x_up1_conv)
        
        return x_out
class UNetAuthor(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(UNetAuthor, self).__init__()
...
#maybe try maxpool for downsampling
#upsampling try 
...
class UNetRes2(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='maxpool', upsample_mode='convtranspose'):
        super(UNetRes2, self).__init__()
        ...
        self.m_up3 = B.sequential(upsample_block(nc[3], nc[2], bias=False, mode='4'), *[B.ResBlock(nc[2], nc[2], bias=False, mode='C'+act_mode+'C') for _ in range(nb)])
        self.m_up2 = B.sequential(upsample_block(nc[2], nc[1], bias=False, mode='4'), *[B.ResBlock(nc[1], nc[1], bias=False, mode='C'+act_mode+'C') for _ in range(nb)])
        self.m_up1 = B.sequential(upsample_block(nc[1], nc[0], bias=False, mode='4'), *[B.ResBlock(nc[0], nc[0], bias=False, mode='C'+act_mode+'C') for _ in range(nb)])
...
class UNetRes3(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='strideconv', upsample_mode='upconv'):
...
class UNetRes4(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='R', downsample_mode='maxpool', upsample_mode='upconv'):
